...
int queue_empty(void) {
	...
	for (prio = 0; prio < MAX_PRIO; prio++)
	{
		if(!empty(&mlq_ready_queue[prio])){
			printf("ben trong if prio %d\n", prio); 
			return -1;
		}
		printf("ben ngoai if prio %d\n", prio);
	}
#endif
...
struct pcb_t * get_mlq_proc(void) {
	...
	if (cnt_slot==0||empty(&mlq_ready_queue[cnt_prio])){
		...
		do{
			...
			if (!empty(&mlq_ready_queue[cnt_prio])){
				...
				cnt_slot = MAX_PRIO-cnt_prio;//cnt_prio?;
